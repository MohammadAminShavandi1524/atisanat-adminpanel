...
interface EditProductImageUploadFieldProps {
  currentImage?: string;
  value?: File;
...
const EditProductImageUploadField = ({
  currentImage,
  value,
  onChange,
  error,
  ...
  isFinalizing,
  disabled = false,
}: EditProductImageUploadFieldProps) => {
  ...
  const previewUrl = newPreviewUrl || currentImage || null;
  ...
  return (
    <div>
      ...
      <div className="grid items-stretch gap-5 lg:grid-cols-[220px_minmax(0,1fr)]">
        <div
          role="button"
          ...
          onDrop={handleDrop}
          className={cn(
            "border-border bg-secondary-bg/25 relative h-full min-h-[250px] overflow-hidden rounded-xl border border-dashed transition-[border-color,background-color] duration-300",
            disabled
            ...
            <>
              ...
              {value && !disabled && (
                <button
                  ...
                  aria-label={t("form.image.remove")}
        ...
        <div className="border-border bg-secondary-bg/25 flex min-h-[250px] flex-col justify-between rounded-xl border p-5">
          <div>
            ...
            <div className="border-border mt-5 border-t pt-4">
              <div className="text-muted-foreground space-y-1.5 text-xs">
                <p>{t("form.image.format")}</p>

                <p>{t("form.image.maxSize")}</p>

                <p>{t("form.image.ratio")}</p>
              </div>
            </div>
          </div>
            ...
            <div className="mt-5">
              <div className="mb-2 flex items-center justify-between gap-3">
                <span className="text-muted-foreground text-xs">
                  {isFinalizing
                    ? t("form.image.finalizing")
                    : t("form.image.uploading")}
                </span>
...
export default EditProductImageUploadField;
