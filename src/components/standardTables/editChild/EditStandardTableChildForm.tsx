...
import { uploadChartFile } from "@/components/standardTables/addChild/add-child-chart.api";

import EditParentChartSelect from "./EditParentChartSelect";

import EditChartSizeSelect from "./EditChartSizeSelect";

import EditChartPdfUploadField from "./EditChartPdfUploadField";

import {
  editChildChartSchema,
  type EditChildChartFormValues,
} from "./edit-child-chart.schema";

import { getChildChartById, updateChildChart } from "./edit-child-chart.api";

interface EditStandardTableChildFormProps {
  childId: string;
}

const EditStandardTableChildForm = ({
  childId,
}: EditStandardTableChildFormProps) => {
  ...
  } = useForm<EditChildChartFormValues>({
  ...
  useEffect(() => {
    const fetchChild = async () => {
      try {
        const data = await getChildChartById(childId);

        const parentId =
          typeof data.chart === "number" ? data.chart : (data.chart?.id ?? 0);
        ...
        console.error("GET CHILD CHART ERROR:", error);
    ...
    fetchChild();
  }, [childId, reset, t, toast]);
  
  const onSubmit = async (data: EditChildChartFormValues) => {
    try {
      const payload: {
        chart: number;
        name_en: string;
      ...
      await updateChildChart(childId, payload);

      toast.success(t("toast.success"));

      router.push(`/${locale}/standard-tables`);
    } catch (error) {
      console.error("UPDATE CHILD CHART ERROR:", error);
        ...
        >
          <div className="3xl:gap-y-8 flex flex-col gap-y-8 pb-8 xl:gap-y-6 2xl:gap-y-7">
            ...
            <Controller
              ...
              name="size"
              render={({ field }) => (
                <EditChartSizeSelect field={field} error={errors.size} />
              )}
            />
...
export default EditStandardTableChildForm;
